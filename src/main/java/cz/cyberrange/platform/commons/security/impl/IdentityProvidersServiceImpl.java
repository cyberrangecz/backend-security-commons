...
import static cz.cyberrange.platform.commons.security.util.JsonUtils.getAsString;
import static cz.cyberrange.platform.commons.security.util.JsonUtils.getAsStringList;
...
public class IdentityProvidersServiceImpl implements IdentityProvidersService {
  ...
  public WellKnownOpenIDConfiguration getIdentityProviderConfiguration(String provider) {
    try {
      ...
      logger.warn("Couldn't load configuration for {}: {}", provider, var3.toString());
      return null;
    }
  }
  ...
  private static class JwksResponse {
    @NonNull private List<Map<String, Object>> keys = new ArrayList<>();
  }
